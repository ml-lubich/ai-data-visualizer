...
| Flask API (backend) | `python3 server/app.py` | 5001 | Proxies to Claude API; requires `ANTHROPIC_API_KEY` env var |
...
- **Fallback demo mode**: When `ANTHROPIC_API_KEY` is not set, the backend returns hardcoded
